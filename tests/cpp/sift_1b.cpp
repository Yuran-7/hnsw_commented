...
static float
test_approx(
    unsigned char *massQ,   // 10000*128
    size_t vecsize, // 2亿
    size_t qsize,   // 10000
    HierarchicalNSW<int> &appr_alg, // 向量索引
    size_t vecdim,  // 128
    // answers是一个数组，数组的每个元素是一个队列，队列的每个元素是一个pair，pair(距离，向量标签)
    vector<std::priority_queue<std::pair<int, labeltype>>> &answers,    
    size_t k) {
    size_t correct = 0;
    size_t total = 0;
    ...
    for (int i = 0; i < qsize; i++) {   // 1万条查询向量
        std::priority_queue<std::pair<int, labeltype >> result = appr_alg.searchKnn(massQ + vecdim * i, k);
        std::priority_queue<std::pair<int, labeltype >> gt(answers[i]); // 从 answers 数组中复制第 i 个查询的 Ground Truth
...
static void
test_vs_recall(
    unsigned char *massQ,   // 存储查询向量数组，10000*128
    size_t vecsize,     // 2亿
    size_t qsize,   // 10000
    ...
    for (size_t ef : efs) {
        appr_alg.setEf(ef); // 测试不同的ef对实验的影响
        StopW stopw = StopW();

        float recall = test_approx(massQ, vecsize, qsize, appr_alg, vecdim, answers, k);
        float time_us_per_query = stopw.getElapsedTimeMicro() / qsize;  // 计算每次查询的平均时间
...
void sift_test1B() {
    ...
    size_t vecsize = subset_size_milllions * 1000000;   // 2亿个数据
    ...
    unsigned int *massQA = new unsigned int[qsize * 1000];  // 1万条查询语句，每一条都有1000个真实最邻近
    for (int i = 0; i < qsize; i++) {
        int t;
        inputGT.read((char *) &t, 4);   // 读取第一个四字节的数据放到t中，t的值为1000
        inputGT.read((char *) (massQA + 1000 * i), t * 4);  // 读取后面1000个四字节数据放入
        if (t != 1000) {
            cout << "err";
            return;
        }
...
#pragma omp critical
    ...
    cout << "Loaded gt\n";
    for (int i = 0; i < 1; i++)
        test_vs_recall(massQ, vecsize, qsize, *appr_alg, vecdim, answers, k);   // 测试召回率
    cout << "Actual memory usage: " << getCurrentRSS() / 1000000 << " Mb \n";
    return;
}
